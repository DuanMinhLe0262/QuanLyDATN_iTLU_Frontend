...
const UploadHuongDanForm = ({ onCancel }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await axios.post("http://localhost:8080/huong-dan/import", formData, {
      ...
      // Giả định backend trả về dạng:
      // { successList: [...], errorList: [...] }

      if (res.data.errorList && res.data.errorList.length > 0) {
        setErrors(res.data.errorList.map(err => `❌ Dòng ${err.row}: ${err.message}`));
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center">
      ...
      <div className="relative w-3/5 h-4/5 bg-white rounded-xl z-50 p-6 overflow-y-auto shadow-lg">
        ...
        <button
          ...
          className="bg-blue-600 text-white px-4 py-2 mt-5 rounded-lg hover:bg-blue-700 disabled:opacity-50"
        >
